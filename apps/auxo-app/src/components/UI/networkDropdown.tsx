...
import { chainMap, isChainSupported, SUPPORTED_CHAIN_ID } from '../../utils/networks';

const ChainAndLogo = ({ chain }: { chain: NetworkDetail | null }) => (
...
export const ChainMenuItem = ({ chain, chainId, id }: { chain: NetworkDetail | null, chainId: number | undefined, id: string }) => (
    <Menu.Item>
        {({ active }) => (
            <button
                onClick={() => changeNetwork({ chainId: Number(id) })}
                className={`${active ? 'bg-baby-blue-light text-baby-blue-dark' : 'text-gray-900'
...
export default function NetworkSwitcher() {
    ...
    const chain = isChainSupported(chainId) ? chainMap[chainId as SUPPORTED_CHAIN_ID] : null;
    return (
        <div className="w-56 text-right z-20">
            <Menu as="div" className="text-left">
                <div>
                    <Menu.Button className="inline-flex items-center justify-between w-full px-3 py-2 
                        shadow-md 
                        ...
                        <div className="px-1 py-1">
                            {
                                Object.entries(chainMap).map(([id, chain]) => (
                                    <ChainMenuItem key={id} chain={chain} id={id} chainId={chainId} />
                                ))
                            }
                        </div>
                    </Menu.Items>
