...
const toState = ({ existing, address, decimals, data, account }: {
  existing: Vault,
  address: string,
  decimals: number,
   ...
   */
  const toNumber = (n: BigNumber | BigNumber[]): number => (n as BigNumber).toNumber()
  let returnValue = {
    ...existing,
    stats: {
  ...
  if (account) returnValue = {
    ...
    userBalances: {
      ...
      batchBurn: {
        ...
        available: returnValue.userBalances?.batchBurn.available ?? toBalance(0, decimals)
      }
    },
  }
  if (data[9]) {
...
const hasStateChanged = (old: Vault[], change: Vault[]): boolean => {
  const hasChanged = hash(old, { encoding: 'base64' }) !== hash(change, { encoding: 'base64' })
  return hasChanged;
}
  ...
  useEffect(() => {
    if (
      ...
      Promise.all(
        ...
        const newVaults = payload.filter(p => !!p).map(p => {
          return p && toState({
            existing: p.vault,
            address: p.address,
