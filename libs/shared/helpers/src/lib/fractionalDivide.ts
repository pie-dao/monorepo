...
*/ 
export const scaleUp = (n: number, decimals = 18): BigNumber => ethers.utils.parseUnits(n.toString(), decimals);
...
**/
export const scaleDown = (b: BigNumberish, decimals = 18): string => ethers.utils.formatUnits(b, decimals);
 ...
 */
export const fractionalNumberDivide = (numerator: number, denominator: number, decimals = 18): number => {
    const [bigNumerator, bigDenomiator] = [numerator, denominator].map(x => scaleUp(x));    
    const bigOutcome = fractionalBNDivide(bigNumerator, bigDenomiator)
